const UI = {
    ...
    renderLogs: function() {
        // studyData가 없거나 logs가 없으면 빈 객체 사용
    ...
    renderLogDetail: function(date) {
        // 데이터가 없으면 강제로 빈 구조 생성 (에러 방지 핵심)
        const log = (window.studyData.logs && window.studyData.logs[date]) ? window.studyData.logs[date] : { chats: [], sentences: [] };
