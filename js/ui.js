const UI = {
    ...
    renderLogs: function() {
        ...
        if (dates.length === 0) html += `<p style="text-align:center; color:#999; margin-top:50px;">데이터가 없습니다.</p>`;
    ...
    renderLogDetail: function(date) {
        const log = window.studyData.logs[date] || { chats: [], sentences: [] };
        ...
        const area = this.getContentArea();
        area.innerHTML = html;
        area.scrollTop = area.scrollHeight; // 저장 시 아래로 스크롤
    },
